...
public final class DataStore {
    ...
    private void migrate() {
        ...
        try {
            ...
            db.execSQL("CREATE TABLE IF NOT EXISTS scene_summaries(scene_name TEXT PRIMARY KEY,word_fingerprint TEXT NOT NULL,summary TEXT NOT NULL,raw_response TEXT NOT NULL DEFAULT '',updated_at TEXT NOT NULL)");
    ...
    public JSONArray allScenes()throws Exception{LinkedHashMap<String,JSONObject> grouped=new LinkedHashMap<>();try(Cursor c=db.rawQuery("SELECT s.name,s.batch_id,sw.word,sw.reason,w.content FROM scenes s JOIN import_batches b ON b.id=s.batch_id JOIN scene_words sw ON sw.scene_id=s.id LEFT JOIN words w ON w.word=sw.word WHERE b.status IN ('draft','confirmed') ORDER BY s.name COLLATE NOCASE,s.position,sw.word",null)){while(c.moveToNext()){String name=c.getString(0),word=c.getString(2);JSONObject scene=grouped.get(name);if(scene==null){scene=new JSONObject().put("name",name).put("batch_id",c.getLong(1)).put("members",new JSONArray());grouped.put(name,scene);}JSONArray members=scene.getJSONArray("members");boolean exists=false;for(int i=0;i<members.length();i++)if(word.equals(members.getJSONObject(i).getString("word"))){exists=true;break;}if(!exists){JSONObject content=c.isNull(4)?new JSONObject():new JSONObject(c.getString(4));members.put(new JSONObject().put("word",word).put("reason",c.getString(3)).put("meaning",content.optString("meaning")).put("explanation",content.optString("explanation")).put("example",content.optString("example")));}}}JSONArray out=new JSONArray();for(JSONObject scene:grouped.values()){JSONArray members=scene.getJSONArray("members");StringBuilder fp=new StringBuilder();for(int i=0;i<members.length();i++)fp.append(members.getJSONObject(i).getString("word")).append('|');scene.put("fingerprint",Integer.toHexString(fp.toString().hashCode()));out.put(scene);}return out;}
    public JSONObject sceneSummary(String name,String fingerprint)throws Exception{try(Cursor c=db.rawQuery("SELECT summary,raw_response FROM scene_summaries WHERE scene_name=? AND word_fingerprint=?",new String[]{name,fingerprint})){return c.moveToFirst()?new JSONObject(c.getString(0)).put("_raw",c.getString(1)):null;}}
    public void saveSceneSummary(String name,String fingerprint,JSONObject summary,String raw){ContentValues v=new ContentValues();v.put("scene_name",name);v.put("word_fingerprint",fingerprint);v.put("summary",summary.toString());v.put("raw_response",raw);v.put("updated_at",LocalDateTime.now().toString());db.insertWithOnConflict("scene_summaries",null,v,SQLiteDatabase.CONFLICT_REPLACE);}
    public void saveMemoryTurn(String word,String question,String answer,JSONObject result,String raw){ContentValues v=new ContentValues();v.put("word",word);v.put("question",question);v.put("answer",answer);v.put("result",result.toString());v.put("raw_response",raw);v.put("created_at",LocalDateTime.now().toString());db.insertOrThrow("memory_chat",null,v);}
    public JSONArray memoryTurns(String word,int limit)throws Exception{JSONArray out=new JSONArray();try(Cursor c=db.rawQuery("SELECT question,answer,result,raw_response,created_at FROM memory_chat WHERE word=? ORDER BY id DESC LIMIT ?",new String[]{word,Integer.toString(limit)})){ArrayList<JSONObject> reverse=new ArrayList<>();while(c.moveToNext())reverse.add(new JSONObject().put("question",c.getString(0)).put("answer",c.getString(1)).put("result",new JSONObject(c.getString(2))).put("raw",c.getString(3)).put("created_at",c.getString(4)));for(int i=reverse.size()-1;i>=0;i--)out.put(reverse.get(i));}return out;}
